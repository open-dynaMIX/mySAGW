...
def test_table_summary(
    ...
    # save third row question and test quoting
    save_answer(
        value='bar;baz\r\nlorem "ipsem"; \'dolor\n',
    ...
    # save first row question to second row document
...
def test_table_summary_errors(
    ...
    assert (
        caplog.messages[1]
        == f'Updating table summary: summary mode "missing" does not exist. Must be one of {settings.TABLE_SUMMARY_MODES}'
    )
